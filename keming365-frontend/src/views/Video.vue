  ...
  <div class="video-page">
    <div class="main-content">
      ...
      <div class="course-section">
        <div class="section-title">{{ currentCourse ? t(currentCourse, currentLang) : '请选择课程分类' }}</div>
        <div class="course-grid">
          <template v-if="pageCourses.length">
            <div v-for="(c, i) in pageCourses" :key="i" class="course-item" @click="openCourse(c.url)">
              ...
              <div class="course-name">{{ t(c.title, currentLang) }}</div>
            </div>
          </template>
...
import { countries, courseCategories, getCourses, t } from '@/data/videoCourses'
...
const currentCourse = ref('机械设计基础A')
...
const allCourses = computed<VideoCourse[]>(() => {
  ...
  return getCourses(currentCountry.value, currentCourse.value)
})
...
const selectCountry = (code: string) => {
  ...
  currentPage.value = 1
  // 自动选中第一个课程
  const cats = courseCategories[code] || []
  if (cats.length > 0) {
    currentCourse.value = cats[0]
  }
}

// 页面加载时自动选中第一个课程
onMounted(() => {
  const cats = courseCategories[currentCountry.value] || []
  if (cats.length > 0) {
    currentCourse.value = cats[0]
  }
})
...
.video-pagination { margin: 30px 0; }
