...
from apps.courses.serializers import ExperimentListSerializer
...
class FeaturedExperimentListView(APIView):
    """Return the curated homepage experiments stored in ``tb_hot``."""

    permission_classes = [AllowAny]

    def get(self, request, *args, **kwargs):
        class_id = request.query_params.get('classId', '').strip()
        if not class_id:
            return Response(
                {'detail': 'classId is required'},
                status=status.HTTP_400_BAD_REQUEST,
            )

        try:
            page_size = min(max(int(request.query_params.get('page_size', 5)), 1), 100)
        except (TypeError, ValueError):
            page_size = 5

        hot_items = list(
            TbHot.objects.filter(classId=class_id, status=1)
            .exclude(eid__isnull=True)
            .exclude(eid='')
            .order_by('sortOrder')
        )
        experiment_ids = [item.eid for item in hot_items]
        experiments_by_id = {
            str(item.id): item
            for item in TbExperiment.objects.filter(id__in=experiment_ids)
        }
        ordered_experiments = [
            experiments_by_id[str(item.eid)]
            for item in hot_items
            if str(item.eid) in experiments_by_id
        ]
        results = ExperimentListSerializer(
            ordered_experiments[:page_size],
            many=True,
            context={'request': request},
        ).data
        return Response({
            'count': len(ordered_experiments),
            'next': None,
            'previous': None,
            'results': results,
        })
