...
OSS_PPT_PREVIEW_ENDPOINT = 'https://www.keming365.com/ai/api/doc-preview/oss/path'
...
def _ppt_filename_from_value(value: str) -> str:
    """Normalize a PPT query value to a single filename."""
    if not value:
        return ''
    raw_value = value.strip()
    if raw_value.startswith(('http://', 'https://')):
        raw_value = urlparse(raw_value).path
    name = unquote(raw_value.replace('\\', '/').split('/')[-1])
    return name


def _ppt_oss_object_from_source(source: str) -> str:
    """Resolve a public keming365 PPT URL to its OSS object path."""
    if not source or not source.startswith(('http://', 'https://')):
        return ''
    parsed = urlparse(source.strip())
    path = unquote(parsed.path.lstrip('/'))
    if path.lower().endswith(('.ppt', '.pptx')) and path.startswith(('file/PPT/', 'node/courseWare/')):
        return path
    return ''


def _ppt_preview_unavailable_html(source: str) -> str:
    download_link = ''
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; }}
    ...
    .text {{ color: #666; margin-bottom: 18px; }}
    .btn {{
...
<body>
  <div class="box">
    <div class="title">课件暂未接入在线预览</div>
    <div class="text">该 PPT 未找到可用的真实 OSS 预览对象，请同步课件后再预览。</div>
    {download_link}
  </div>
</body>
</html>'''
...
def _is_valid_oss_preview_url(preview_url: str) -> bool:
    if not preview_url:
        return False
    try:
        ...
        if resp.status_code >= 400:
            return False
        text = resp.text[:500]
        return 'NoSuchKey' not in text and 'Weboffice' in text
    except Exception as exc:
        logger.warning('OSS PPT preview validation failed: url=%s error=%s', preview_url, exc)
        return False
...
@api_view(['GET'])
@permission_classes([AllowAny])
def ppt_oss_preview(request):
    """
    GET /api/v1/files/ppt-oss-preview/

    Resolve a courseware PPT filename through the legacy OSS document preview
    service, then redirect the iframe to the returned preview URL.
    """
    file_name = _ppt_filename_from_value(request.GET.get('file', ''))
    source = request.GET.get('source', '').strip()

    if not file_name:
        file_name = _ppt_filename_from_value(source)

    if not file_name or Path(file_name).suffix.lower() not in {'.ppt', '.pptx'}:
        return Response(
            {'code': 'INVALID_PPT_FILE', 'message': '缺少有效的 PPT 文件名'},
            status=status.HTTP_400_BAD_REQUEST,
        )

    object_candidates = [f'node/courseWare/{file_name}', _ppt_oss_object_from_source(source)]
    seen_objects = set()
    for object_name in [item for item in object_candidates if item]:
        if object_name in seen_objects:
            continue
        seen_objects.add(object_name)
        try:
            resp = requests.post(
                OSS_PPT_PREVIEW_ENDPOINT,
                json={'objectName': object_name},
                timeout=15,
            )
            resp.raise_for_status()
            data = resp.json()
            preview_url = data.get('data') if isinstance(data, dict) else ''
            if _is_valid_oss_preview_url(preview_url):
                return HttpResponseRedirect(preview_url)
            logger.warning('OSS PPT preview unavailable: file=%s object=%s response=%s', file_name, object_name, data)
        except Exception as exc:
            logger.warning('OSS PPT preview failed: file=%s object=%s error=%s', file_name, object_name, exc, exc_info=True)

    return HttpResponse(_ppt_preview_unavailable_html(source), content_type='text/html; charset=utf-8')
