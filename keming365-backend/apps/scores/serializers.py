...
class ExperimentScoreSerializer(serializers.ModelSerializer):
    ...
    usedTime = serializers.SerializerMethodField(read_only=True)
    ...
    def get_experimentName(self, obj) -> str:
        ...
        # 优先返回数据库中已存储的名称
        experiment_str = getattr(obj, 'experimentStr', None)
        if experiment_str:
    ...
    def get_totalScore(self, obj):
        """操作分"""
        return obj.operationScore

    def get_usedTime(self, obj):
        """用时。旧表没有独立用时字段时返回空值。"""
        return getattr(obj, 'useTime', None)
