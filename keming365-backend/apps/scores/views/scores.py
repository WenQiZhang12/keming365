...
from utils.user_roles import is_teacher_or_admin
...
class ExperimentScoreDetailView(APIView):
    ...
    返回指定实验的成绩详情：
      - 教师（type=1）和管理员（type=4）可以查看所有用户的成绩
      - 学生（type=2）及普通用户只能查看自己的成绩
    ...
    def get(self, request, pk):
        ...
        # 权限校验：学生只能看自己的记录
        if not is_teacher_or_admin(request.user) and score_obj.userId != user_id:
            raise BusinessError(
                message='无权查看其他用户的成绩',
