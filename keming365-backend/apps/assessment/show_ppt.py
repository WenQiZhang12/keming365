...
ACCESS_KEYS = []
for access_key_config in os.environ.get('YUNQ_ACCESS_KEYS', '').split(','):
    access_key, separator, secret = access_key_config.strip().partition(':')
    if separator and access_key and secret:
        ACCESS_KEYS.append((access_key, secret))
...
def _try_start_url(appli_id: str, yunq_url: str = DEFAULT_YUNQ_URL) -> dict:
    """依次尝试多组密钥，直到成功"""
    if not ACCESS_KEYS:
        return {'success': False, 'error': 'YUNQ_ACCESS_KEYS is not configured'}
